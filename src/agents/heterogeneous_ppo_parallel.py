"""
Parallel Heterogeneous multi-agent training for the Bangalore MG Road corridor.

Uses multiprocessing to spin up 4 parallel SUMO instances. 5 independent PPO 
...
def train_parallel(
    ...
    models = {
        t: PPO(
            ...
            facades[t], 
            verbose=0, 
            n_steps=360,     # Max steps is 1800 delta=5 -> 360 agent decisions per episode
            batch_size=60, 
            learning_rate=3e-4,
            gamma=0.99
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--run-name", type=str, default="blr-parallel-ppo")
