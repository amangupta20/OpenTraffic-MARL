...
```
\n## 4. Feudal MARL Architecture\n\n### 4.1 Hierarchy\n\n1.  **Macro Manager**: Operates every 60s (`MANAGER_C = 4` logical steps) observing the true Global State. Outputs continuous goal dimensions `g_i ∈ [-1, 1]` for each intersection via Zero-Order Hold.\n2.  **Micro Worker (CTDE)**: Operates every 15s observing Local State and Manager's `g_i`. Executed via MAPPO.\n\n### 4.2 Reward Structure\n\n**Manager Reward:** Only observes the environmental global step reward `(sum(per-junction reward) / 100)`. Receives credit mapped flawlessly across `MANAGER_C` step delays due to strided PPO GAE unwinding.\n\n**Worker Reward:**\n```python\nr_ext = environment_junction_reward\nr_int = -(g_q * (queue_length / 20.0) + g_w * (wait_time / 300.0))\nworker_reward = ALPHA * r_ext + (1 - ALPHA) * r_int\n```\nWhere `ALPHA = 0.5`, balancing global coordination signals against local throughput.\n\n### 4.3 Parallel Training Infrastructure\n\nTo decouple Manager delays without cross-environment trajectory contamination within PPO Rollout Arrays:\n1.  **Shared Subprocesses:** 4 `cologne8_corridor` instances run in parallel Python processes.\n2.  **Interlacing Array Write:** Workers save arrays chronologically: `[E0_T0, E1_T0, E2_T0...]`.\n3.  **Strided GAE Bootstrap:** Advantages are calculated by stepping backwards dynamically dynamically indexing `t+num_envs` to accurately preserve per-process state transitions.\n
